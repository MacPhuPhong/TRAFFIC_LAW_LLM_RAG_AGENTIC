...
def rq8_figures():
    ...
    import json
    pipelines = [("gemini_only", "tab:green", "Gemini-only"),
                 ("vanilla_rag", "tab:blue", "Vanilla RAG"),
                 ("agentic_rag", "tab:red", "Agentic RAG")]
    data = {}
    all_lats = []
    ...
    if all_lats:
        # Shared log-spaced bins so the agentic_rag outlier (~1200s) doesn't
        # stretch the bin width and swallow the other two distributions.
        bins = np.logspace(np.log10(max(min(all_lats), 0.1)),
                            np.log10(max(all_lats)), 16)
        fig, axes = plt.subplots(len(pipelines), 1, figsize=(9, 7), sharex=True)
        for ax, (name, color, label) in zip(axes, pipelines):
            if name not in data:
                ax.set_visible(False)
                continue
            ax.hist(data[name], bins=bins, color=color, alpha=0.85)
            ax.set_ylabel("Count")
            ax.set_title(label, fontsize=10, loc="left")
        axes[-1].set_xscale("log")
        axes[-1].set_xlabel("Latency (s, thang log)")
        fig.suptitle("RQ8 — Per-pipeline latency distribution (25-question eval)")
        fig.tight_layout()
