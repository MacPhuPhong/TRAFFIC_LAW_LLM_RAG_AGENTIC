...
def load_gold() -> list[dict]:
    # Legal-only: 35 questions (drop 5 out_of_scope — empty gold_citations
    # trivializes recall metrics).
...
def main():
    ...
    p.add_argument("--retrieval-only", action="store_true",
                   help="Skip the vanilla RAG (Gemini) pass — faster, no API.")
    args = p.parse_args()

    gold = load_gold()
    log.info("Loaded %d gold questions (legal-only)", len(gold))

    log.info("=== HIERARCHICAL (e5-base, Traffic_Law_Hybrid) ===")
    ...
    if args.retrieval_only:
        log.info("--retrieval-only flag set; skipping vanilla RAG pass.")
