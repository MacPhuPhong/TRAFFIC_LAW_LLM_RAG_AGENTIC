...
QDRANT_COLLECTION = "Traffic_Law_Hybrid"
...
def main():
    ...
    _all = [json.loads(l) for l in open(GOLD, encoding="utf-8") if l.strip()]
    gold = [r for r in _all
            if r.get("expected_category") != "out_of_scope"
            and r.get("category") != "out_of_scope"]
    log.info("Loaded %d legal-only questions (filtered from %d)", len(gold), len(_all))
    rss0 = rss_mb()
    ...
    log.info("Loading %s for query encoding", EMBEDDING_MODEL)
    model = SentenceTransformer(EMBEDDING_MODEL)
    query_prefix = "query: " if "e5" in EMBEDDING_MODEL.lower() else ""
    queries_vec = model.encode(
        [query_prefix + g["question"] for g in gold], normalize_embeddings=True,
